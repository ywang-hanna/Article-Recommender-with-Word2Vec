...
def load_glove(filename):
    ...
    for elem in contents:
        words = elem.split(" ")
        myDict[words[0]] = np.array(words[1:len(words)], dtype = "float")
    return myDict
...
def load_articles(articles_dirname, gloves):
    ...
    for file in allfiles:
        filename = file.replace(articles_dirname+'/', '')
        content = get_text(file)
        contents = content.split('\n', 1)
        title = contents[0]
        text = contents[1]
        centroid = doc2vec(content, gloves)
        record.append((filename, title, text, centroid))
    return record
...
def doc2vec(text, gloves):
    ...
    """
    word = words(text)
    word = [w for w in word if w in gloves]
    word_vec = [gloves[w] for w in word]
    centroid = np.mean(list(word_vec), axis=0)
    return centroid
...
def distances(article, articles):
    ...
    distance = [(np.linalg.norm(article[-1] - elem[-1]), elem) for elem in articles2]
    return distance
...
def recommended(article, articles, n):
    ...
    closest = sorted(distance, key = lambda x: x[0])[:n]
    output = [close[1] for close in closest]
    return output
...
#     article = articles[2]
#
#     print(recommended(article, articles, 5))
